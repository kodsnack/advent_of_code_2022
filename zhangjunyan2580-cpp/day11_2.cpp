...
inline void apply(int &v, int op, int value) {
    if (value == -1)
        switch (op) {
            case ADD: v <<= 1; (v >= M) && (v -= M); break;
            case SUB: v = 0; break;
            case MUL: v = (long long) v * v % M; break;
        }
    else
        switch (op) {
            case ADD: v += value; (v >= M) && (v -= M); break;
            case SUB: v -= value; (v < 0) && (v += M); break;
            case MUL: v = (long long) v * value % M; break;
        }
}
...
void round() {
    for (int i = 0; i < n; ++i) {
        ...
        for (int &v : monkey.items) {
            apply(v, monkey.update_op, monkey.update_value);
...
int main() {
    ...
    for (int i = 0; i < 10000; ++i) round();
    int m1 = 0, m2 = 0;
    for (int v : inspect_count)
        if (v > m1) m2 = m1, m1 = v;
        else if (v > m2) m2 = v;
    std::cout << (long long) m1 * m2 << std::endl;
    return 0;
}