...
int dp[31][65536][16], a[100][100], val[100], n, m, ans;
...
int main() {
    ...
    dp[0][0][ints(strs("AA"))] = 0;
    int mS = 1 << m;
    for (int t = 0; t < 30; ++t)
        for (int S = 0; S < mS; ++S)
            for (int i = 0; i < m; ++i) {
                int v = dp[t][S][i];
                if (!((S >> i) & 1)) upd(S | (1 << i), t + 1, i, v + (29 - t) * val[ints[i]]);
                for (int j = 0; j < m; ++j)
                    if (i != j && t + a[ints[i]][ints[j]] <= 30) upd(S, t + a[ints[i]][ints[j]], j, v);
            }
